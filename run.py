...
sim_sizes = {'test':'./evaluation/evaluation --kpvm-dram 1 4 128 1024 8 8 7 1000 1', 
            ...
            'large' : './evaluation/evaluation --kpvm-dram 2 16 128 1024 8 8 7 400000 4'}
...
#                help='workloads to be run: tpcc/ycsb')
    ...
    if args.sim_size in sim_sizes:
        exportCmd = 'export LD_LIBRARY_PATH=./malloc/lib/:../lib/:$LD_LIBRARY_PATH'
        runCmd(exportCmd, "not found", "Error finding libraries!");
        deleteCmd = 'rm -rf /dev/shm/efile'
        runCmd(deleteCmd, "Error", "Error deleting previous memory pool!");
        runCmd(sim_sizes[args.sim_size], "no free memory", "Simulation failed")    
    os.chdir('../../')
